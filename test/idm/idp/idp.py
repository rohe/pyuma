...
def get_eptid(idp, resp_args, session):
    return idp.eptid.get(idp.config.entityid,
                         resp_args["sp_entity_id"], session["uid"],
                         authn_auth())
...
class Service(object):
    ...
    def operation(self, _dict, binding, session):
    ...
    def do(self, query, binding, relay_state="", session=None):
        pass

    def redirect(self, session):
        """ Expects a HTTP-redirect request """

        _dict = self.unpack_redirect()
        session.update(dict)
        return self.operation(_dict, BINDING_HTTP_REDIRECT, session)
    ...
    def post(self, session):
        ...
        _dict = self.unpack_post()
        session.update(dict)
        return self.operation(_dict, BINDING_HTTP_POST, session)
    ...
    def soap(self, session):
        ...
        session.update(dict)
        logger.debug("_dict: %s" % _dict)
        return self.operation(_dict, BINDING_SOAP, session)

    def uri(self, session):
        _dict = self.unpack_either()
        session.update(dict)
        return self.operation(_dict, BINDING_SOAP, session)
...
class SSO(Service):
    ...
    def _authn_response(self, resp, relay_state):
        logger.info("AuthNResponse: %s" % resp)
        http_args = IDP.apply_binding(self.binding_out,
                                      "%s" % resp, resp.destination,
...
#            identity[REPOZE_ID_EQUIVALENT] = self.user
    ...
    def do(self, query, binding_in, relay_state="", session=None):
        try:
        ...
        if not _resp:
            # Contact the RS to get user info
            _state = rndstr(16)
            IDP.cache.spq[_state] = {"resp_args": resp_args,
                                     "relay_state": relay_state,
                                     "req_info": self.req_info,
                                     "binding_out": self.binding_out,
                                     "destination": self.destination}
            user_resp = USERS(UIDMAP[self.user], resp_args["sp_entity_id"],
                              state=_state)
    ...
    def redirect(self, session):
        ...
        try:
            ...
            session["sp_entity_id"] = _req.issuer.text
            ...
            if self.user:
                if _req.force_authn:
                    ...
                    return self.operation(_info, BINDING_HTTP_REDIRECT, session)
            else:
                _info["req_info"] = self.req_info
                key = self._store_request(_info)
                return self.not_authn(key, _req.requested_authn_context)
    ...
    def post(self, session):
        ...
        if self.user:
            if _req.force_authn:
                ...
                return self.operation(_info, BINDING_HTTP_POST, session)
        else:
            _info["req_info"] = self.req_info
            key = self._store_request(_info)
            return self.not_authn(key, _req.requested_authn_context)
    ...
    # After authorization request to the AS
    def authz_cb(self, session):
        try:
            aresp = AuthorizationResponse().from_urlencoded(
                self.environ["QUERY_STRING"])
            owner, requestor = USERS.get_tokens(
                self.environ["QUERY_STRING"]).split("@")
            resp = USERS.get_info(owner, requestor)
            if isinstance(resp, oicResponse):
                if resp.status == "200 OK":
                    resp = json.loads(resp.message)["resource"]
                else:
                    return resp(self.environ, self.start_response)

            sess = IDP.cache.spq[aresp["state"]]
            self.destination = sess["destination"]
            self.binding_out = sess["binding_out"]
            return self._identity(resp, sess["resp_args"], sess["relay_state"],
                                  session)
...
def username_password_authn(environ, start_response, reference, key,
    ...
    try:
        argv["sp_entity_id"] = environ["idp.sp_entity_id"]
    except KeyError:
        argv["sp_entity_id"] = ""
...
class SLO(Service):
    def do(self, request, binding, relay_state="", session=None):
...
# Only URI binding
class AIDR(Service):
    def do(self, aid, binding, relay_state="", session=None):
...
def application(environ, start_response):
    ...
    try:
        ...
        user = ""
    try:
        environ["idp.authn_ref"] = session["authn_ref"]
    except KeyError:
        pass
    ...
    for regex, callback in url_patterns:
        ...
        if match is not None:
            ...
            if isinstance(callback, tuple):
                ...
                return callback(environ, start_response, user, session)
...
if __name__ == '__main__':
    ...
    USERS = UMAUserInfo(BASE, ["%s/authz_cb" % BASE], "https://localhost:8089",
                        acr="BasicAuthn", verify_ssl=False)
